...
if ($result->num_rows > 0) {
    ...
    $sql_check_survey = "SELECT survey_id FROM m_survey WHERE survey_jenis = 'Alumni' AND survey_nama = '$survey_nama' AND user_id IS NULL LIMIT 1";
    ...
    if ($result_check_survey->num_rows > 0) {
        ...
        if ($conn->query($sql_update_survey) === TRUE) {
            ...
            $sql_responden_id = "SELECT responden_alumni_id, survey_id FROM t_responden_alumni WHERE responden_nama = '$nama' ORDER BY responden_alumni_id ASC";
            ...
            if ($result_responden_id->num_rows > 0) {
                ...
                while ($row_responden = $result_responden_id->fetch_assoc()) {
                    $responden_id = $row_responden['responden_alumni_id'];
                    if ($row_responden['survey_id'] == null) {
                        // Update responden kosong dengan survey_id baru
                        $sql_update_responden = "UPDATE t_responden_alumni SET survey_id = '$available_survey_id', responden_tanggal = NOW() WHERE responden_alumni_id = '$responden_id'";
                ...
                if (!$found_empty) {
                    ...
                    echo '<a href="../user/alumni/surveyPage.php" class="buttonK">Kembali</a>'; // Perubahan pada tombol
                    echo '</div>';
                }
            } else {
                echo '<link rel="stylesheet" href="../style.css">';
