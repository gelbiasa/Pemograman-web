...
if ($result->num_rows > 0) {
    ...
    $sql_check_survey = "SELECT survey_id FROM m_survey WHERE survey_jenis = 'Dosen' AND survey_nama = '$survey_nama' AND user_id IS NULL LIMIT 1";
    ...
    if ($result_check_survey->num_rows > 0) {
        ...
        if ($conn->query($sql_update_survey) === TRUE) {
            ...
            $sql_responden_id = "SELECT responden_dosen_id, survey_id FROM t_responden_dosen WHERE responden_nama = '$nama' ORDER BY responden_dosen_id ASC";
            ...
            if ($result_responden_id->num_rows > 0) {
                ...
                while ($row_responden = $result_responden_id->fetch_assoc()) {
                    ...
                    if ($row_responden['survey_id'] == null) {
                        ...
                        $sql_update_responden = "UPDATE t_responden_dosen SET survey_id = '$available_survey_id', responden_tanggal = NOW() WHERE responden_dosen_id = '$responden_id'";
                ...
                if (!$found_empty) {
                    ...
                    foreach ($_POST as $key => $value) {
                        if (substr($key, 0, 6) === 'answer') { // Jika input adalah jawaban
                            ...
                            $sql_insert_jawaban = "INSERT INTO t_jawaban_dosen (responden_dosen_id, soal_id, jawaban) VALUES ('$responden_id', '$soal_id', '$jawaban')";
                    ...
                    echo '<a href="../user/dosen/surveyPage.php" class="buttonK">Kembali</a>'; // Perubahan pada tombol
                    echo '</div>';
                }
            } else {
                echo '<link rel="stylesheet" href="../style.css">';
