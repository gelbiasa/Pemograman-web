            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($row['soal_jenis'] == "Pilihan") {
                        ...
                        // Query to get the total number of text answers
                        $jawaban_text_count_sql = "SELECT COUNT(*) AS total_jawaban
                                                   FROM $table t
                                                   INNER JOIN m_survey_soal sv ON t.soal_id = sv.soal_id
                                                   INNER JOIN m_survey s ON sv.survey_id = s.survey_id
                                                   WHERE t.soal_id = " . $row['soal_id'] . " AND s.survey_nama = '$survey_nama' AND s.survey_jenis = '$user_status'";
                        
                        $jawaban_text_count_result = $conn->query($jawaban_text_count_sql);
                        $total_jawaban = $jawaban_text_count_result->fetch_assoc()['total_jawaban'];

                        // Query to get the first text answer
                        ...
                        if ($jawaban_text_result->num_rows > 0) {
                            ...
                            echo "<p id='jawabanTerbaru'>Jawaban Terbaru: " . $jawaban_text_row['jawaban'] . "</p>";
                            echo "<button id='nextButton' data-soal-id='" . $row['soal_id'] . "' data-current-index='1' data-total-jawaban='$total_jawaban' data-survey-nama='$survey_nama' data-user-status='$user_status'>Next</button>";
                        } else {
                            echo "Tidak ada jawaban untuk soal ini.";
                        }
                    }
