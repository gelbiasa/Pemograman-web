...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'accept') {
        $sql = "SELECT * FROM t_pending_mahasiswa WHERE pending_user_id = $id";
        ...
        if ($result->num_rows > 0) {
            ...
            if ($conn->query($sql1) === TRUE) {
                ...
                VALUES ('$nama', '$responden_nim', '$responden_email', '$responden_hp', '$responden_prodi', '$tahun_masuk')";
                ...
                if ($success) {
                    $conn->query("DELETE FROM t_pending_mahasiswa WHERE pending_user_id = $id");
                    echo "Accepted";
                } else {
